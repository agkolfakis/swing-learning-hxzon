...
public class ValueBasedGradientHighlighter extends PainterHighlighter {
    ...
    protected boolean canHighlight(Component component, ComponentAdapter adapter) {
        return (adapter.getValue() instanceof Number) && super.canHighlight(component, adapter);
    }
    ...
    public void setYellowTransparent(boolean yellowTransparent) {
        if (this.yellowTransparent == yellowTransparent)
            return;
    ...
    private Painter getPainter(float end) {
        if (getPainter().getPainter() == null) {
            Color startColor = getTransparentColor(Color.RED, yellowTransparent ? 125 : 254);
            Color endColor = getTransparentColor(Color.RED.brighter(), 0);
            GradientPaint paint = new GradientPaint(new Point2D.Double(0, 0), endColor, new Point2D.Double(1000, 0), startColor);
