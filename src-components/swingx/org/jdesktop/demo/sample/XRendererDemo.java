...
public class XRendererDemo {
    ...
    private static final Logger LOG = Logger.getLogger(XRendererDemo.class.getName());
    ...
    private void prepareRenderering() {
        ...
        LazyIconValue windIconValue = new LazyIconValue("compass/", ".png", null, null);
        iconValues.put("WIND_DIR", windIconValue);
        StringValue sv = new StringValue() {

            public String getString(Object value) {
                String key = value instanceof String ? (String) value : StringValues.TO_STRING.getString(value);
                key = key.toLowerCase();
                key = key.replace(' ', '-');
                return key;
            }
        ...
        LazyIconValue flagIconValue = new LazyIconValue("flags/", ".gif", sv, new EmptyIcon(30, 19));
        ...
        AbstractHighlighter temperature = new ColorHighlighter(new AndHighlightPredicate(new IdentifierHighlightPredicate("TEMPERATURE"), HighlightPredicate.INTEGER_NEGATIVE), null, Color.BLUE);
        ...
        stationTreeTable.setHighlightPredicate(new AndHighlightPredicate(new IdentifierHighlightPredicate("COUNTRY"), HighlightPredicate.ROLLOVER_ROW));
    ...
    private CustomColumnFactory createAndConfigureColumnFactory() {
        ...
        factory.addExcludeNames("REGION", "ICAO", "TIMESTAMP", //"DEWPOINT", "HUMIDITY",
                "VISIBILITY_QUAL", "VISIBILITY", "GUST_SPEED");
        factory.addHiddenNames("LATITUDE", "LONGITUDE", "DEWPOINT", "HUMIDITY", "WIND_DEG");
        StringValue sv = new StringValue() {
    ...
    private void configureTable(JXTable table, TableModel weather, ColumnFactory factory) {
        ...
        table.getColumnExt("COUNTRY").setCellRenderer(new DefaultTableRenderer(new MappedValue(StringValues.TO_STRING, iconValues.get("COUNTRY"))));
        table.getColumnExt("WIND_DIR").setCellRenderer(new DefaultTableRenderer(new MappedValue(null, iconValues.get("WIND_DIR")), JLabel.CENTER));
        // add highlighters
        // tooltips
        ...
        for (int i = 0; i < table.getColumnCount(); i++) {
            Component comp = table.prepareRenderer(table.getCellRenderer(0, i), 0, i);
        ...
        margin = 10;
        int max = -1;
        int viewIndex = table.convertColumnIndexToView(table.getColumnExt("COUNTRY").getModelIndex());
        table.packColumn(viewIndex, margin, max);

    }

    private void configureTreeTable(WeatherTreeTableModel model, JXTreeTable treeTable, ColumnFactory factory, int rowHeight, int rowCount) {
...
//        treeTable.setShowGrid(false, true);
        ...
        treeTable.setTreeCellRenderer(new DefaultTreeRenderer(iconValues.get("COUNTRY"), sv));

        treeTable.getColumnExt("WIND_DIR").setCellRenderer(new DefaultTableRenderer(new MappedValue(null, iconValues.get("WIND_DIR")), JLabel.CENTER));

        // add highlighters
        // color for parent (country only) nodes
        treeTable.addHighlighter(new ColorHighlighter(new DepthHighlightPredicate(1), HighlighterFactory.FLORAL_WHITE, null));
        // tooltips
...
//        treeTable.packTable(margin);
        ...
        int margin = 10;
        int max = -1;
        int viewIndex = treeTable.convertColumnIndexToView(treeTable.getColumnExt("COUNTRY").getModelIndex());
    ...
    class WeatherTreeTableModel extends DefaultTreeTableModel {
        ...
        public WeatherTreeTableModel(WeatherTableModel source) {
            ...
            if (stationColumn < 0)
                throw new IllegalStateException("no station column");
        ...
        private MutableTreeTableNode findParent(DefaultMutableTreeTableNode root, Object country) {
    ...
    class WeatherTableModel extends DefaultTableModel {
        String[] ids = { "ICAO", "STATION", "REGION", "COUNTRY", "ELEVATION", "LATITUDE", "LONGITUDE", "TIMESTAMP", "TEMPERATURE", "DEWPOINT", "HUMIDITY", "VISIBILITY_QUAL", "VISIBILITY", "WIND_DIR",
                "WIND_DEG", "WIND_SPEED", "GUST_SPEED" };
        Class<?>[] classes = { null, null, null, null, Float.class, Float.class, Float.class, Date.class, Float.class, Float.class, Integer.class, null, Float.class, null, Integer.class,
        ...
        public Class<?> getColumnClass(int columnIndex) {
            return classes[columnIndex] == null ? super.getColumnClass(columnIndex) : classes[columnIndex];
        }
