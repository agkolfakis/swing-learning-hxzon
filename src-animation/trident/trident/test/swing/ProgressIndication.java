...
public class ProgressIndication extends JFrame {
    public static class ProgressPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Inner gradient fill
            Shape innerContour = new RoundRectangle2D.Double((w - INNER_WIDTH) / 2.0, (h - INNER_HEIGHT) / 2.0, INNER_WIDTH, INNER_HEIGHT, 10, 10);
            g2d.setPaint(new GradientPaint((w - INNER_WIDTH) / 2.0f, (h - INNER_HEIGHT) / 2.0f, new Color(47, 47, 47), (w - INNER_WIDTH) / 2.0f, (h + INNER_HEIGHT) / 2.0f, Color.black));
            ...
            for (int i = 2; i >= 0; i--) {
                Shape trackContour = new RoundRectangle2D.Double((w - INNER_WIDTH) / 2 + 18 - i, h / 2 - i - 1, trackWidth + i * 2, trackHeight + i * 2, 2 * i, 2 * i);
                float alpha = 1.0f;
                if (i == 1)
                    alpha = 0.3f;
                if (i == 2)
            ...
            for (int i = 6; i >= 0; i--) {
                g2d.fillOval(this.xPosition - HIGHLIGHTER_WIDTH / 2 - i, h / 2 - HIGHLIGHTER_HEIGHT / 2 - i, HIGHLIGHTER_WIDTH + 2 * i, HIGHLIGHTER_HEIGHT + 2 * i);
            }
