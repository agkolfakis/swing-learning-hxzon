...
public class CropCircles {
    public static class CropCircle {
        ...
        public void paint(Graphics g) {
            ...
            g2d.fill(new Ellipse2D.Float(this.xc - this.radius, this.yc - this.radius, 2 * this.radius, 2 * this.radius));
    ...
    private static class CropArc {
        ...
        public CropArc(float x1, float y1, float x2, float y2, float x3, float y3) {
            ...
            this.radius = (float) Math.sqrt((x1 - this.xc) * (x1 - this.xc) + (y1 - this.yc) * (y1 - this.yc));
        }
        ...
        public void populate(float startAngle, float startRadius, float factor) {
            ...
            while (currRad > 2) {
                double b = Math.abs(Math.acos(1.0 - currRad * currRad / (2 * this.radius * this.radius)));

                currRad *= factor;
                double g = Math.abs(Math.acos(1.0 - currRad * currRad / (2 * this.radius * this.radius)));
                currAngle -= (b + g);
                this.cs.add(new CropCircle(this.xc + this.radius * (float) Math.cos(currAngle), this.yc - this.radius * (float) Math.sin(currAngle), currRad));
            }
        }
    ...
    public static class CropPanel extends JPanel {
        ...
        public CropPanel() {
            ...
            int smallRadius = 15;
            this.center1 = new CropCircle(this.arc1.getCenterX(), this.arc1.getCenterY(), smallRadius);
            this.center1.setRadius(smallRadius);
            this.center2 = new CropCircle(this.arc2.getCenterX(), this.arc2.getCenterY(), smallRadius);
            this.center2.setRadius(smallRadius);
            this.center3 = new CropCircle(this.arc3.getCenterX(), this.arc3.getCenterY(), smallRadius);
            this.center3.setRadius(smallRadius);
            Timeline pulseCenters = new Timeline();
            pulseCenters.addPropertyToInterpolate(Timeline.<Float> property("opacity").on(this.center1).from(0.0f).to(1.0f));
            pulseCenters.addPropertyToInterpolate(Timeline.<Float> property("opacity").on(this.center2).from(0.0f).to(1.0f));
            pulseCenters.addPropertyToInterpolate(Timeline.<Float> property("opacity").on(this.center3).from(0.0f).to(1.0f));
