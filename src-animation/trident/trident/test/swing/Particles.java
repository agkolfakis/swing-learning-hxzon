...
public class Particles {
        ...
        public void compose(Raster src, Raster dstIn, WritableRaster dstOut) {
            ...
            for (int y = 0; y < height; y++) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    int[] result = new int[] { Math.min(255, srcPixel[0] + dstPixel[0]), Math.min(255, srcPixel[1] * srcPixel[0] / 255 + dstPixel[1] * dstPixel[0] / 255),
                            Math.min(255, srcPixel[2] * srcPixel[0] / 255 + dstPixel[2] * dstPixel[0] / 255), Math.min(255, srcPixel[3] * srcPixel[0] / 255 + dstPixel[3] * dstPixel[0] / 255) };

                    dstPixels[x] = ((int) (dstPixel[0] + (result[0] - dstPixel[0]) * alpha) & 0xFF) << 24 | ((int) (dstPixel[1] + (result[1] - dstPixel[1]) * alpha) & 0xFF) << 16
                            | ((int) (dstPixel[2] + (result[2] - dstPixel[2]) * alpha) & 0xFF) << 8 | (int) (dstPixel[3] + (result[3] - dstPixel[3]) * alpha) & 0xFF;
                }
    ...
    public static class Particle {
        ...
        public void paint(Graphics g) {
            ...
            g2d.setPaint(new RadialGradientPaint(0, 0, this.size / 2.0f, new float[] { 0.0f, 0.6f, 1.0f }, new Color[] { this.color, this.color,
                    new Color(this.color.getRed(), this.color.getGreen(), this.color.getBlue(), 96) }));

            g2d.fill(new Ellipse2D.Float(-this.size / 2.0f, -this.size / 2.0f, this.size, this.size));
            ...
            g2d.setPaint(new RadialGradientPaint(0, 0, this.size / 3.0f, new float[] { 0.0f, 1.0f }, new Color[] { new Color(255, 255, 255, 96), new Color(255, 255, 255, 32) }));
            g2d.fill(rh);

            Color c1 = new Color(255, 255, 255, 16);
            Color c2 = new Color(255, 255, 255, 32);
            g2d.setPaint(new LinearGradientPaint(d1, d2, -d1, -d2, new float[] { 0.0f, 0.5f, 1.0f }, new Color[] { c1, c2, c1 }));
            g2d.drawLine((int) d1, (int) d2, -(int) d1, -(int) d2);
            g2d.setPaint(new LinearGradientPaint(-d2, d1, d2, -d1, new float[] { 0.0f, 0.5f, 1.0f }, new Color[] { c1, c2, c1 }));
    ...
    public static class ParticlesPanel extends JPanel {
        ...
        public ParticlesPanel() {
            ...
            mouseTracker.addCallback(new TimelineCallbackAdapter() {
                ...
                public void onTimelinePulse(float durationFraction, float timelinePosition) {
                    ...
                    boolean isInside = (mouseLoc.x >= 0) && (mouseLoc.y >= 0) && (mouseLoc.x < getWidth()) && (mouseLoc.y < getHeight());
                    if (isInside) {
                        int x = mouseLoc.x;
                        int y = mouseLoc.y;
                        if (wasMouseInside) {
        ...
        private synchronized void makeParticles(int x1, int y1, int x2, int y2, int numParticles) {
            ...
            Color[] cs = new Color[] { Color.red, Color.blue, Color.green, Color.magenta, Color.cyan, Color.yellow };
            for (int i = 0; i < numParticles; i++) {
                ...
                final Particle particle = new Particle(startX, startY, size, cs[randomizer.nextInt(cs.length)], startAngle);
        ...
        protected void paintComponent(Graphics g) {
            ...
            if ((offscreen == null) || (offscreen.getWidth() != w) || (offscreen.getHeight() != h)) {
                offscreen = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(w, h, BufferedImage.TRANSLUCENT);
            }
